...
var _class = function () {
  ...
  (0, _createClass3.default)(_class, [{
    ...
    key: 'alias',
    value: function alias(type, path) {
      // 注册类型
      (0, _core.alias)(type, path);
    }
  }, {
    key: 'register',
    ...
    value: function _registerDomain(name, itemType) {
      ...
      if (!domain) return;
      for (var alias in _core.fxData.alias) {
        if (alias.indexOf('/' + itemType + '/')) domain[alias] = alias;
      }
    }
  }, {
    ...
    value: function _registerHandler(name, itemType) {
      ...
      if (handlers !== null) {
        ...
        for (var alias in _core.fxData.alias) {
          if (alias.indexOf('/' + itemType + '/') == -1) continue;
          var messageType = alias.trimRight('handler');
