...
import {provider as snapshotProvider} from '../snapshot';
import aggregate from '../aggregate';
import {storage as domainEventStorage} from '../event';
  ...
  async get(name, id, ...props){
    ...
    if (snapshotProvider && await snapshotProvider.hasSnapshot(name, id)){
      ...
      if (evnts != null && evnts.Count() > 0)
        ...
        throw {code:err.aggregateNotExists, msg:`领域对象(id=${id})未能在数据库中找到.`};
    }
